...
const TasksManagement: React.FC = () => {
  ...
  const fetchBoards = async () => {
    ...
    try {
      ...
      console.log('Fetching boards with token:', token ? 'Token exists' : 'No token');
      ...
      showSnackbar("Error fetching boards: " + (error instanceof Error ? error.message : 'Unknown error'), "error");
      setBoards([]);
    } finally {
  ...
  useEffect(() => {
    console.log('TasksManagement component mounted, token:', token ? 'exists' : 'missing');
    fetchData();
  }, [token]);
      ...
      renderCell: (params) => (
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Edit">
            <IconButton
              size="small"
              onClick={() => handleOpenDialog(params.row as KanbanTask)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
          <Tooltip title="Delete">
            <IconButton
              size="small"
              onClick={() => handleDeleteClick(params.row as KanbanTask)}
              color="error"
            >
              <DeleteIcon />
            </IconButton>
      ...
      <Box>
        ...
        <Box sx={{ mb: 2, p: 2, bgcolor: 'background.paper', borderRadius: 1, border: '1px solid #ddd' }}>
          <Typography variant="body2" color="text.secondary">
            ...
            Token: {token ? '✓ Available' : '✗ Missing'}<br/>
            Total boards: {boards.length}<br/>
            Task boards: {getTaskBoards().length}<br/>
            Boards loading: {boardsLoading ? 'Yes' : 'No'}<br/>
          ...
          {boards.length > 0 && (
            <Box sx={{ mt: 1 }}>
              ...
              {boards.map(board => (
                <Typography key={board.id} variant="caption" display="block" color="text.secondary">
        ...
        >
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
              ...
              <FormControl fullWidth required disabled={!formData.boardId}>
                ...
                >
                  ...
                  {formData.boardId && getColumnsForBoard(formData.boardId).length === 0 && (
                    <MenuItem disabled value="">
          ...
          <DialogActions>
            ...
            <Button
              ...
              variant="contained"
              disabled={!formData.title || !formData.description || !formData.boardId || !formData.columnId}
            >
